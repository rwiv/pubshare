...
import {FilePolicy, FilePolicyCreation, FilePolicyResponse} from "@/client/permission/types";
...
export async function findFilePoliciesByFileId(fileId: number) {
  const res = await axios.get<FilePolicyResponse[]>(`${baseUrl}/file-id/${fileId}`, getHeaderConfig());
  return res.data;
}
